...
static
int ussd_send_data_ss(isup_connection_t *conn, struct ss_request* reply, uint32_t ref);

static
int ussd_send_reject(isup_connection_t *conn, uint32_t ref, uint8_t invoke_id, uint8_t opcode);
...
static int rx_sup_uss_message(isup_connection_t *sup_conn, const uint8_t* data, size_t len)
{
	...
	LOGP(DLCTRL, LOGL_ERROR, "Got ref=%d mtype=0x%02x invoke_id=0x%02x opcode=0x%02x component_type=0x%02x text=%s\n", ref,
	...
	case GSM0480_MTYPE_REGISTER:
		...
		rc = ussd_session_open_mo(op, sup_conn, &ss, ref, extention);
		if (rc < 0) {
...
err_send_reject:
	ussd_send_reject(sup_conn, ref, ss.invoke_id, ss.opcode);
	return -1;
}
...
int ussd_send_reject(isup_connection_t *conn, uint32_t ref, uint8_t invoke_id, uint8_t opcode)
{
	...
	return ussd_send_data_ss(conn, &error_ss, ref);
}
		   ...
		   const char* msg, unsigned msg_len)
{
	...
	return ussd_send_data_ss(op->ussd.conn, &ss, op->ussd.ref);
}
